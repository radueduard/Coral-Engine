...
namespace Coral {
  ...
  //   	Reef::Container<ImageTest> noiseTestContainer = Reef::MakeContainer<ImageTest>(noise.Image());
    	...
    	// m_sceneManager->GetLoadedScene().Root().AddChild(PlaneMeshes::Plane({0.f, 0.f, -25.f}, { 0.3f, 0.3f, 0.3f }));
    	m_sceneManager->GetLoadedScene().Root().AddChild(PlaneMeshes::Plane({0.f, 0.f, 25.f}, { 0.1f, 0.1f, 0.1f }));

    	auto& dirLight = m_sceneManager->GetLoadedScene().Root().AddLight(ECS::LightType::Directional);
    	dirLight.Get<ECS::Transform>().position = Math::Vector3f { 0.f, 0.f, -25.f };
    	dirLight.Get<ECS::Transform>().rotation = Math::Vector3f { 0.f, 180.f, 0.f };
    	dirLight.Get<ECS::Light>().intensity = 5.f;
    	...
    	spot1.Get<ECS::Light>().intensity = 10.f;
    	...
    	spot2.Get<ECS::Light>().intensity = 10.f;
