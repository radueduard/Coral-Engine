...
namespace Coral::PlaneMeshes {
	...
	inline std::unique_ptr<ECS::Entity> Plane(Math::Vector3f position, Math::Vector3f scale = {1.f, 1.f, 1.f}) {
		...
		planeEntity->Get<ECS::Transform>().scale = scale;
		planeEntity->Get<ECS::Transform>().rotation = Math::Vector3f { 90.f, 0.f, 45.f };
		planeEntity->Get<ECS::Transform>().position = position;
